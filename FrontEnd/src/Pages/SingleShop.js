...
export default function SingleShop() {
    ...
    useEffect(async() => {
       ...
       await db.collection("ShopList").doc(postdat.uid).collection('Shop').onSnapshot((snapshot) => {
       ...
       await db.collection("MerchantList").doc(postdat.uid).onSnapshot((snapshot) => {
       ...
       await db.collection("Users").doc(localStorage.getItem('uid')).onSnapshot((snapshot) => {
    ...
    async function checkoutadd() {
        ...
        console.log(jitem)
        axios.post(`${process.env.REACT_APP_API_URL1}/api/User/AddCart`,
            {
                ShopRef:mercuid,
                ShopName:mercname,
                Items:jitem,
