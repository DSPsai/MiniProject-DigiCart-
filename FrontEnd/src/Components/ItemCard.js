...
export default function ItemCard(item) {
    ...
    function plus() {
        let items=localStorage.getItem('items');
        if(items === undefined)localStorage.setItem('items','[{"DocId": "","itemName": "","itemCount": "0"}]')
        let jitem=JSON.parse(`${items}`)
        let found=false;
        for(let i of jitem){
            if(i.DocId == item.ider){
        ...
        if(!found){jitem.push({DocId:item.ider,itemCount:1,itemName:item.itemName})}
        localStorage.setItem('items',JSON.stringify(jitem))
        console.log(jitem,localStorage.getItem('items'))
    ...
    return (
        <div className='itemCard'>
            ...
            </div> :!item.bol? <div className="itemtot">
            ...
            </div>:<div className='openshop' onClick={()=>{history(`/Shop/${item.docid}`)}}>Go To Shop</div>
            }
        </div>
    )
}
